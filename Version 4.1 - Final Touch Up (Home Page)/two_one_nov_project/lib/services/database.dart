...
class DatabaseService {
  ...
  //Health Report
  Future<void> updateHealthReport(DateTime appDate,String medicine,String medicine1,String medicine2,String quantity,String quantity1,String quantity2,String shortComment,String docComment) async {
    final DateFormat formatter = DateFormat('dd-MM-yyyy');
    final String formatAppDate = formatter.format(appDate);
    await personal_Info.document(uid).updateData({
      'short_comment': shortComment,
    });
    return await Firestore.instance.collection("appointment_"+uid).document(formatAppDate).updateData({
      'medicine': medicine,
      'medicine1': medicine1,
      'medicine2': medicine2,
      'quantity': quantity,
  ...
  //Bill Report
  Future<void> updateBillReport(DateTime appDate, String paymentMethod, String paymentSelection, String seriesNum) async {
    final DateFormat formatter = DateFormat('dd-MM-yyyy');
    final String formatAppDate = formatter.format(appDate);
    await personal_Info.document(uid).updateData({
